...
@section('content')
    <div class="flex flex-col gap-2">
        <neo-datavisualizer print search filter download title="{{ __('Brands List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.brands.store') }}" aria-label="create_page_link"
...
@section('scripts')
    <script>
        TableVisualizer(document.querySelector("neo-datavisualizer"), "brands", {
            Search: "{{ route('actions.brands.search') }}",
            Patch: "{{ route('views.brands.patch', 'XXX') }}",
            Clear: "{{ route('actions.brands.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
