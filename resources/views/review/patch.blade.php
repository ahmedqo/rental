...
@section('title', __('Edit Review') . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-2">
        <h1 class="text-center lg:text-start text-xl lg:text-2xl text-x-black font-x-thin">
            {{ __('Edit Review') . ' #' . $data->id }}
        </h1>
        <div class="bg-x-white rounded-x-thin shadow-x-core border border-x-shade p-6">
            <form action="{{ route('actions.reviews.patch', $data->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <neo-autocomplete set-query="{{ 'name_' . Core::lang() }}" set-value="id" label="{{ __('Car') . ' (*)' }}"
                    name="car" value="{{ $data->car }}" query="{{ $data->Car->name }}">
                ...
                <neo-textarea auto="false" label="{{ __('Content') . ' (*)' }}" name="content"
                    value="{{ $data->content }}" rows="5" class="lg:col-span-2"></neo-textarea>
                <neo-datepicker full-day="3" label="{{ __('Date') }}" name="date"
                    value="{{ $data->updated_at->format('Y-m-d') }}" format="mmm dd"></neo-datepicker>
                <neo-timepicker label="{{ __('Time') }}" name="time"
                    value="{{ $data->updated_at->format('h:i') }}"></neo-timepicker>
