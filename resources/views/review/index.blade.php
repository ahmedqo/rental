...
@section('content')
    <div class="flex flex-col gap-2">
        <neo-datavisualizer print search filter download title="{{ __('Reviews List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.reviews.store') }}"
...
@section('scripts')
    <script>
        TableVisualizer(document.querySelector("neo-datavisualizer"), "reviews", {
            Search: "{{ route('actions.reviews.search') }}",
            Patch: "{{ route('views.reviews.patch', 'XXX') }}",
            Clear: "{{ route('actions.reviews.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
