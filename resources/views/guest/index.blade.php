            ...
            <div class="flex flex-col lg:flex-row gap-10 my-6 lg:my-20">
                ...
                <div class="w-full">
                    <form action="{{ route('views.guest.fleet') }}"
                        ...
                        <span class="font-x-thin text-x-black text-3xl lg:text-4xl col-span-2">
                            {{ __('Find Your Car') }}
                        </span>
                        <neo-autocomplete label="{{ __('Location') }}" name="location"
...
@section('scripts')
    ...
    <script>
        ...
        var timer;
        document.querySelector("neo-autocomplete").addEventListener("input", (e) => {
            if (timer) clearTimeout(timer);
            e.target.data = ["Airport", "City Center (Marrakesh)"]
        });
    </script>
@endsection
