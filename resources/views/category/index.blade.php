...
@section('content')
    <div class="flex flex-col gap-2">
        <neo-datavisualizer print search filter download title="{{ __('Categories List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.categories.store') }}"
...
@section('scripts')
    <script>
        TableVisualizer(document.querySelector("neo-datavisualizer"), "categories", {
            Search: "{{ route('actions.categories.search') }}",
            Patch: "{{ route('views.categories.patch', 'XXX') }}",
            Clear: "{{ route('actions.categories.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
